...
import net.sandius.rembulan.lbc.recompiler.gen.block.LuaBinaryOperation;
import net.sandius.rembulan.lbc.recompiler.gen.block.LuaInstruction;
import net.sandius.rembulan.lbc.recompiler.gen.block.LuaUtils;
...
public class JavaBytecodeCodeVisitor extends CodeVisitor {
	...
	public static String binaryOperationMethodName(LuaBinaryOperation.Op op) {
		switch (op) {
			case ADD:  return DispatchMethods.OP_ADD;
			case SUB:  return DispatchMethods.OP_SUB;
			case MUL:  return DispatchMethods.OP_MUL;
			case MOD:  return DispatchMethods.OP_MOD;
			case POW:  return DispatchMethods.OP_POW;
			case DIV:  return DispatchMethods.OP_DIV;
			case IDIV: return DispatchMethods.OP_IDIV;
			case BAND: return DispatchMethods.OP_BAND;
			case BOR:  return DispatchMethods.OP_BOR;
			case BXOR: return DispatchMethods.OP_BXOR;
			case SHL:  return DispatchMethods.OP_SHL;
			case SHR:  return DispatchMethods.OP_SHR;
	...
	protected InsnList binaryNumericOperation(LuaBinaryOperation.Op op, SlotState s, int rk_left, int rk_right) {
		...
		il.add(DispatchMethods.numeric(binaryOperationMethodName(op), 2));
	...
	protected InsnList binaryDynamicOperation(LuaBinaryOperation.Op op, SlotState s, int rk_left, int rk_right) {
		...
		il.add(DispatchMethods.dynamic(binaryOperationMethodName(op), 2));
	...
	public static String comparisonMethodName(LuaInstruction.Comparison op) {
		switch (op) {
			case EQ: return DispatchMethods.OP_EQ;
			case LT: return DispatchMethods.OP_LT;
			case LE: return DispatchMethods.OP_LE;
	...
	protected InsnList numericComparison(LuaInstruction.Comparison op, int rk_left, int rk_right, boolean pos, SlotState s, LabelNode falseBranch) {
		...
		il.add(e.loadRegisterOrConstant(rk_left, s, Number.class));
		il.add(e.loadRegisterOrConstant(rk_right, s, Number.class));
		il.add(DispatchMethods.numeric(comparisonMethodName(op), 2));
	...
	protected InsnList dynamicComparison(LuaInstruction.Comparison op, int rk_left, int rk_right, boolean pos, SlotState s, LabelNode falseBranch) {
		...
		il.add(e.loadRegisterOrConstant(rk_left, s));
		il.add(e.loadRegisterOrConstant(rk_right, s));
		il.add(DispatchMethods.dynamic(comparisonMethodName(op), 2));
